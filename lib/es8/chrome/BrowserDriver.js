...
class BrowserDriver {
	...
	_initialize(callback) {
		...
		//if (this._options.extraDebug) {
		//	childOptions.push("--enable-logging=stderr")
		//	childOptions.push("--v=1")
		//}
		...
		child.on("exit", (code, signal) => {
			if (signal) {
				process.stdout.write(`Fatal: Chrome subprocess killed by signal ${signal}\n`)
			} else {
				process.stdout.write(`Fatal: Chrome subprocess exited with code ${code}\n`)
			}
			process.exit(42) // arbitrary code different than 1 or 0
		})
