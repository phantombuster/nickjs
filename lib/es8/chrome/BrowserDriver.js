...
class BrowserDriver {
	...
	_initialize(callback) {
		...
		const proxyServer = process.env.HTTP_PROXY || process.env.http_proxy || null
		const childOptions = [
		...
		// Yay: http://2ality.com/2017/04/conditional-literal-entries.html
		if (proxyServer) {
			childOptions.push(`--proxy-server=${proxyServer}`);
		}
