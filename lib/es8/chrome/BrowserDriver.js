...
class BrowserDriver {
	...
	_initialize(callback) {
		...
		let proxyServer = process.env.NICKJS_PROXY || process.env.HTTP_PROXY || process.env.http_proxy
		if (proxyServer) {
			// we need a protocol for correct parsing of username/password
			if (proxyServer.indexOf("//") < 0 || proxyServer.indexOf("//") > 20) {
				proxyServer = `http://${proxyServer}`
			}
			const { URL } = require('url');
			const url = new URL(proxyServer)
			this._options.proxyUsername = url.username
			this._options.proxyPassword = url.password
			console.log("proxy auth is " + this._options.proxyUsername + ':' + this._options.proxyPassword)
			console.log("setting proxy server to " + url.host)
			childOptions.push(`--proxy-server=${url.host}`)
		}
		if (boolean(process.env.NICKJS_NO_SANDBOX)) {
			console.log("add no sandbox flag")
