...
class TabDriver {
	...
	_init(callback) {
		Promise.all([
			...
			// enable request interception only if necessary
			if ((this.__options.whitelist.length > 0) || (this.__options.blacklist.length > 0) || this.__options._proxyUsername || this.__options._proxyPassword) {
			...
			// (only do this if we have URLs to block or proxy authentication to provide)
			if ((this.__options.whitelist.length > 0) || (this.__options.blacklist.length > 0) || this.__options._proxyUsername || this.__options._proxyPassword) {
				const continueRequest = (auth, e, error, reason) => {
					if (reason && this.__options.printAborts) {
						console.log(`> Tab ${this.id}: Aborted (${reason}): ${e.request.url}`)
					}
					const payload = {
					...
					if (auth) {
						payload.authChallengeResponse = auth
					}
				...
				this.__client.Network.requestIntercepted((e) => {
					...
					if (this.__options.whitelist.length > 0) {
						...
						if (!found) {
							return continueRequest(auth, e, "Aborted", "not found in whitelist")
						}
					}
					for (const black of this.__options.blacklist) {
						if (typeof black === 'string') {
							...
							if ((url.indexOf(black) === 0) || (url.indexOf(`https://${black}`) === 0) || (url.indexOf(`http://${black}`) === 0)) {
								return continueRequest(auth, e, "Aborted", `blacklisted by "${black}"`)
							}
						} else if (black.test(e.request.url)) {
							return continueRequest(auth, e, "Aborted", `blacklisted by ${black}`)
						}
					}
					continueRequest(auth, e)
				})
			}
			...
			if (err) {
				...
				const frameNavigated = (e) => {
					if (e.frame.id === frameId) {
						...
						//console.log(status)
						//if ([null, 204, 205, 304, 407].includes(status)) {
						//	// for these HTTP codes, Chrome will never fire a domContentEvent
						//	// so we return immediately
						//	callback(null, status, statusText, newUrl)
						//	return
						//}
						if (this.__options.printNavigation) {
							console.log(`> Tab ${this.id}: Navigation (open): ${newUrl}`)
						}
	...
	_screenshot(filename, options, callback) {
		// TODO allow for null filename => return base64
	...
	_deleteCookie(name, domain, callback) {
		...
		this.__client.Network.deleteCookie(payload, (err, res) => {
