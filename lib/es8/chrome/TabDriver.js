...
class TabDriver {
	...
	_init(callback) {
		Promise.all([
			...
			this.__client.Security.certificateError((e) => {
				this.__client.Security.handleCertificateError({
					eventId: e.eventId,
					action: 'continue'
				});
			});
				...
				const frameNavigated = (e) => {
					if (e.frame.id === frameId) {
						...
						if (this.__options.printNavigation) {
							console.log(`> Tab ${this.__uniqueTabId}: Navigation: ${newUrl}`)
						}
	...
	_getAllCookies(callback) {
		this.__client.Network.getAllCookies({}, (err, res) => {
			...
			if (err) {
				...
				callback(null, res.cookies) // TODO check this
			}
		})
	}
	...
	_deleteCookie(name, domain, callback) {
		...
		this.__client.Network.deleteCookie(payload, (err, res) => {
			callback(this.__parseCdpResponse(err, res, "deleteCookie: failed to delete cookie"))
		})
	}
	...
	_setCookie(cookie, callback) {
		...
		this.__client.Network.setCookie(payload, (err, res) => {
			callback(this.__parseCdpResponse(err, res, "setCookie: could not add cookie"))
		})
	}
