...
class TabDriver {
	...
	constructor(uniqueTabId, options, client, cdpTargetId, parseCdpResponse) {
		this.__closed = false
		this.__uniqueTabId = uniqueTabId
		this.__options = options
		this.__client = client
		this.__cdpTargetId = cdpTargetId
		this.__parseCdpResponse = parseCdpResponse
		...
		Promise.all([
			...
			this.__client.Security.certificateError((e) => {
				...
				this.__client.Security.handleCertificateError(payload, (err, res) => {
					...
					if (err) {
						console.log(`> Tab ${this.__uniqueTabId}: ${err}`)
					}
				})
			})
			...
			if (this.__options.printNavigation) {
				this.__client.Page.frameScheduledNavigation((e) => {
					console.log(`> Tab ${this.__uniqueTabId}: Navigation (${e.reason}): ${e.url}`)
				})
			}
			...
			if (err) {
				...
				const frameNavigated = (e) => {
					if (e.frame.id === frameId) {
						...
						//}
						if (this.__options.printNavigation) {
							console.log(`> Tab ${this.__uniqueTabId}: Navigation (open): ${newUrl}`)
						}
