...
class Tab {
	...
	set onDialog(f) {
		if (typeof(f) !== "function") {
			throw new TypeError("onDialog must receive a function(callback)")
		}
		this._onDialog = f
	}
	...
	isVisible(selectors, operator = null, callback = null) {
		...
		const f = (callback) => {
			waitUntilVisible(selectors, 1, operator, (err) => {
	...
	isPresent(selectors, operator = null, callback = null) {
		...
		const f = (callback) => {
			waitUntilPresent(selectors, 1, operator, (err) => {
				callback(null, !Boolean(err))
			})
		}
		if (callback) {
	...
	deleteCookie(name, domain, callback = null) {
		...
		if (typeof domain !== 'string') {
			throw new TypeError('domain parameter must be of type string')
		}
